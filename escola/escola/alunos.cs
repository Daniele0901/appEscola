            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        cmd.Parameters.AddWithValue("@idade", string.IsNullOrWhiteSpace(novaidade) ? reader["idade"] : (novaidade));
                        cmd.Parameters.AddWithValue("@curso", string.IsNullOrWhiteSpace(novocurso) ? reader["curso"] : novocurso);
                        cmd.Parameters.AddWithValue("@datamatricula", string.IsNullOrWhiteSpace(novadatamatriucla) ? reader["datamatricula"] : (novadatamatriucla));
                        cmd.Parameters.AddWithValue("@Id", idEditar);
